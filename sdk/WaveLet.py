...
class WaveLet():
    ...
    """
    主要功能：用于分析同一室内多个传感器的小波网络
    出入数据：（假设室内有m个传感器，每个传感器取此刻向前n个时间单位的传感器值）
    ...
    def __init__(self, batch_size,output_size,input_x,input_y,wavelet_amount):
        ...
        with tf.name_scope('inputs'):
            ...
            self.label_holder = tf.placeholder(tf.int32, [batch_size,output_size], name='label')

        # 添加网络主体
        with tf.name_scope("wavelet_body"):
            self.add_layer()
        ...
        with tf.name_scope('train'):
            self.train_op = tf.train.AdamOptimizer(1e-3).minimize(self.loss)  # 0.72
    ...
    def wavelet_morlet(self,t):
        return math.exp(-t*t/2)*math.cos(1.75*t)

    # 定义小波网络部分（使用小波对图片每一行进行处理）
    def wavelet_pro_row(self,row):
        ...
        # “输入”经过权重矩阵后再经小波函数
        w_pro = tf.map_fn(fn=lambda x:self.wavelet_morlet(x),elems=tf.matmul(row,w_ij))
